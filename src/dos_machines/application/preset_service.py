...
from dos_machines.domain.models import AppPaths, MachinePreset
...
class PresetService:
    ...
    def load_machine_presets(self) -> list[MachinePreset]:
        ...
        return [
            self._normalize_machine_preset(MachinePreset.from_json(item))
            for item in payload.get("machine_presets", [])
        ]
    ...
    def load_system_machine_presets(self) -> list[MachinePreset]:
        return [
            self._normalize_machine_preset(preset) for preset in SYSTEM_MACHINE_PRESETS
        ]

    def load_section_default(
        self, engine_id: str, section_name: str
    ) -> dict[str, str] | None:
        payload = self._load_payload()
    ...
    def save_section_default(
        self, engine_id: str, section_name: str, values: dict[str, str]
    ) -> dict[str, str]:
        payload = self._load_payload()
    ...
    def save_machine_preset(
        ...
        payload = self._load_payload()
        machine_presets = [
            MachinePreset.from_json(item) for item in payload.get("machine_presets", [])
        ]
        existing_machine = next(
            (preset for preset in machine_presets if preset.title == title), None
        )
        ...
        machine_preset = MachinePreset(
            preset_id=existing_machine.preset_id
            if existing_machine is not None
            else f"machine-{uuid4().hex[:12]}",
            title=title,
            source="user",
            sections={
    ...
    def resolve_machine_preset(self, preset_id: str) -> dict[str, dict[str, str]]:
        ...
        if machine_preset.source == "system":
            for section_name, values in self._load_system_machine_preset_sections(
                machine_preset.preset_id
            ).items():
    ...
    def _load_payload(self) -> dict[str, object]:
        if not self._user_presets_path.exists():
            return {"machine_presets": [], "section_defaults": {}}
    ...
    def _load_system_machine_preset_sections(
        self, preset_id: str
    ) -> dict[str, dict[str, str]]:
        asset = resources.files("dos_machines").joinpath(
            "assets", "system-presets", f"{preset_id}.conf"
        )
